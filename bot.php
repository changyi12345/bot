...
/* ========= CFG ========= */
$BOT="7279743667:AAHbJKx12tWP5nY_zWF6TG-6fdl1NHCOls8";//BOT TOken
...
if(!file_exists(TFILE)) file_put_contents(TFILE,"[]",LOCK_EX); // topup requests
...
function home_txt($bal){
  return "🟡 <b>𝐁𝐄𝐄 𝐀𝐮𝐭𝐨 𝐓𝐎𝐏-𝐔𝐏 𝐁𝐎𝐓</b>\n━━━━━━━━━━━━━━━━━━\n".
         "⚡️ ငွေရှိယုံဖြင့် 24နာရီကြိုက်တဲ့အချိန် ဝယ်ယူလို့ရပါတယ်ရှင့်\n\n".
         "🤖 𝗕𝗢𝗧 𝗨𝗽𝗱𝗮𝘁𝗲 𝗖𝗵𝗮𝗻𝗻𝗲𝗹\n @BEE_Game_Shop\n\n".
         "💰 𝐖𝐚𝐥𝐥𝐞𝐭 𝐀𝐦𝐨𝐮𝐧𝐭𝐬 : <b>".h(mmk($bal))."</b>\n".
         "👤 Admin : <b>".h(TOPUP)."</b>\n\n".
         "⏬<b>မီနူးရွေးပါရှင့်</b>";
}
function kmenu(){return ["inline_keyboard"=>[
  ...
  [["text"=>"📋 Topup History","callback_data"=>"SYS:TOPUP_HIS"]],
]];}
...
function ktop(){return ["inline_keyboard"=>[
  ...
  [["text"=>"⬅️ BACK","callback_data"=>"NAV:HOME"],["text"=>"🏠 HOME","callback_data"=>"NAV:HOME"]],
]];}
function kcmd(){return ["inline_keyboard"=>[
  [["text"=>"1","callback_data"=>"CMD:1"],["text"=>"2","callback_data"=>"CMD:2"],["text"=>"3","callback_data"=>"CMD:3"]],
  [["text"=>"4","callback_data"=>"CMD:4"],["text"=>"5","callback_data"=>"CMD:5"],["text"=>"6","callback_data"=>"CMD:6"]],
  [["text"=>"7","callback_data"=>"CMD:7"],["text"=>"8","callback_data"=>"CMD:8"],["text"=>"9","callback_data"=>"CMD:9"]],
  [["text"=>"10","callback_data"=>"CMD:10"],["text"=>"11","callback_data"=>"CMD:11"],["text"=>"12","callback_data"=>"CMD:12"]],
  [["text"=>"⬅️ BACK","callback_data"=>"NAV:HOME"],["text"=>"🏠 HOME","callback_data"=>"NAV:HOME"]],
]];}
...
if(!is_array($tups)) $tups=[];

// create / update user (keep existing ban, balance, states)
if(!isset($users[$cid])){
  $users[$cid]=["bal"=>0.0,"st"=>"","tmp"=>[]];
}

// save telegram username for admin list
$uname=$u["message"]["from"]["username"]??($u["callback_query"]["from"]["username"]??"");
if($uname!==""){
  $users[$cid]["uname"]=(string)$uname;
}
jw(UFILE,$users);

// 🔒 check ban
if(!empty($users[$cid]["ban"])){
  send($cid,"❌ <b>Account သည် Admin မှ ထိန်းချုပ်ထားပါတယ်</b>\nအကူအညီလိုပါက Admin - ".h(TOPUP)." ကို ဆက်သွယ်ပါရှင့်.");
  exit;
}
...
/* ========= ID CHECK COMMANDS ========= */
if($txt){
  if(preg_match('/^(@cmd|\/cmd)\b/i',$txt)){
    send($cid,"📋 <b>Command Menu (1–12)</b>\n━━━━━━━━━━━━━━━━━━\nခေါ်မည့် number ကို အောက်က Button နဲ့ ရွေးချယ်ပါ။",kcmd()); exit;
  }
...
if($cb){
  ...
  if($cb==="NAV:TOPUP"){
    $users[$cid]["st"]="WAIT_TOPUP_AMT";
    $users[$cid]["tmp"]=[];
    jw(UFILE,$users);
    editm($cid,$mid,"💸 <b>Topup / ငွေဖြည့်ရန်</b>\n━━━━━━━━━━━━━━━━━━\n\nထည့်မယ့် amount ကို MMK နဲ့ ရိုက်ထည့်ပါ\nဥပမာ: <code>5000</code> ၊ <code>10000</code> ၊ <code>15000</code>",ktop());
    exit;
  }

  if(preg_match('/^CMD:(\d{1,2})$/',$cb,$m)){
    $n=(int)$m[1];
    if($n<1 || $n>12){ editm($cid,$mid,home_txt($users[$cid]["bal"]),kmenu()); exit; }
    editm($cid,$mid,"✅ <b>Selected Command</b>\n━━━━━━━━━━━━━━━━━━\nYou chose: <b>$n</b>\n\nအခု command နံပါတ်ကို သင့် usage အလိုက် သုံးနိုင်ပါတယ်။",kcmd()); exit;
  }
  ...
  if($cb==="SYS:HIS"){
    ...
    $x=end($list);
    editm($cid,$mid,"📜 <b>HISTORY</b>\n━━━━━━━━━━━━━━━━━━\nTime: ".h($x["time"]??"-")."\nGame: ".h($x["game"]??"-")."\nItem: ".h($x["order"]??"-")."\nSN: <code>".h($x["sn"]??"-")."</code>",["inline_keyboard"=>[[["text"=>"🏠 HOME","callback_data"=>"NAV:HOME"]]]]); exit;
  }
  ...
  if($cb==="SYS:TOPUP_HIS"){
    ...
    $my_tups=[];
    foreach($tups as $tp){
      $tp_cid=(string)($tp["cid"]??"");
      $my_cid=(string)$cid;
      if($tp_cid===$my_cid) $my_tups[]=$tp;
    }
    if(!$my_tups){
      editm($cid,$mid,"📋 <b>Topup History</b>\n━━━━━━━━━━━━━━━━━━\n— No topup history —\n\nTopup request မရှိသေးပါ။\n💸 Topup / ငွေဖြည့်ရန် menu မှတဆင့် Topup လုပ်နိုင်ပါတယ်။",["inline_keyboard"=>[[["text"=>"🏠 HOME","callback_data"=>"NAV:HOME"]]]]); exit;
    }
    usort($my_tups,function($a,$b){ return strcmp($b["time"]??"",$a["time"]??""); });
    $txt="📋 <b>Topup History</b>\n━━━━━━━━━━━━━━━━━━\n\n";
    $i=0;
    foreach($my_tups as $tp){
      if(++$i>10) break;
      $s=$tp["status"]??"pending";
      $icon=$s==="approved"?"✅":($s==="rejected"?"❌":"⏳");
      $txt.="$icon <b>".h(mmk($tp["amount"]??0))."</b> | Ref: <code>".h($tp["ref"]??"-")."</code> | ".h(ucfirst($s))."\n";
      $txt.="   <i>".h($tp["time"]??"-")."</i>\n\n";
    }
    if(count($my_tups)>10) $txt.="\n<i>(Showing last 10 records)</i>";
    editm($cid,$mid,$txt,["inline_keyboard"=>[[["text"=>"🏠 HOME","callback_data"=>"NAV:HOME"]]]]); exit;
  }
  ...
  if(preg_match('/^ORD:(ML|UC|CODE):PAGE:(\d+)$/',$cb,$m)){
    ...
    editm($cid,$mid,$title."  𝐏𝐫𝐢𝐜𝐞𝐬 𝐋𝐢𝐬𝐭\n𝐖𝐚𝐢𝐭𝐢𝐧𝐠 𝐭𝐢𝐦𝐞 𝟎𝐒𝐞𝐜 / မစောင့်ရပါရှင့် ❤️",klist($list,$mode,$page)); exit;
  }
  ...
  if(preg_match('/^ORD:(ML|UC|CODE):CONFIRM$/',$cb,$m)){
    ...
    foreach($codes as $code){
      if($mode==="ML"){
        ...
        $r=code_order($code,$qty); // ✅ CODE uses item_id now
    ...
    if($err){ editm($cid,$mid,"❌ <b>ORDER FAIL</b>\n━━━━━━━━━━━━━━━━━━\n<code>".h($err)."</code>",kmenu()); exit; }
...
/* ========= STATES ========= */
if($st==="WAIT_TOPUP_AMT" && $txt){
  if(!preg_match('/^\d{3,10}$/',$txt)){
    send($cid,"❌ Amount ကို ကိန်းဂဏန်းနဲ့သာ ရိုက်ထည့်ပါ (ဥပမာ <code>5000</code>)",ktop()); exit;
  }
  $amt=(int)$txt;
  if($amt<500){
    send($cid,"❌ အနည်းဆုံး 500 MMK နှင့် အထက် Topup လုပ်နိုင်ပါမယ်",ktop()); exit;
  }
  $users[$cid]["st"]="WAIT_TOPUP_REF";
  $users[$cid]["tmp"]=[
    "top_amt"=>$amt,
    "created"=>time()
  ];
  ...
  $body="💸 <b>Topup Order</b>\n━━━━━━━━━━━━━━━━━━\n".
        "Amount : <b>".h(mmk($amt))."</b>\n".
        "UID    : <code>$cid</code>\n\n".
        "💳 <b>Bank / KBZPay လွှဲရန်</b>\n".
        "Bank Name : KPay\n".
        "Acc Name  : KageRan\n".
        "Acc No    : 112233\n\n".
        "ငွေလွှဲပြီးပြီဆိုရင် Bank ပြေစာပေါ်ကနောက်ဆုံး ၆လုံး Ref/ID ကို ဒီနေရာမှာပို့ပေးပါ\n".
        "ဥပမာ: <code>123456</code>";
  send($cid,$body,ktop()); exit;
}

if($st==="WAIT_TOPUP_REF" && $txt){
  if(!preg_match('/^\d{6}$/',$txt)){
    send($cid,"❌ ပြေစာ ID သည် ၆လုံးကိန်းဂဏန်းသာ ပို့ပေးပါ (ဥပမာ <code>123456</code>)",ktop()); exit;
  }

  $amt=(int)($users[$cid]["tmp"]["top_amt"]??0);
  if($amt<=0){
    $users[$cid]["st"]=""; $users[$cid]["tmp"]=[]; jw(UFILE,$users);
    send($cid,"⏳ Session လွန်သွားပါပြန်\n\nTopup menu မှတဆင့် သို့မဟုတ် /start နဲ့ ပြန်စတင်ပေးပါ",ktop()); exit;
  }
  ...
  send($cid,"✅ <b>Topup Request ပေးပို့ပြီးပါပြီ</b>\n━━━━━━━━━━━━━━━━━━\n".
            "Amount : <b>".h(mmk($amt))."</b>\n".
            "Ref ID : <code>".h($txt)."</code>\n\n".
            "Admin မှ စစ်ဆေးပြီးသွားတာနဲ့ Wallet ထဲသို့ ငွေဝင်သွားမည်ဖြစ်ပါသည်။",ktop()); exit;
}
